...
// https://github.com/mattn/go-sqlite3 ; revision cf7286f069c3ef596efcc87781a4653a2e7607bd

// MODIFIED: removed TestFunctionDeclaration, TestDeclTypes, BenchmarkCustomFunctions,
// ADDED: init func as helper
...
// compatibility helper BEGIN
type CompatHelper sqlite

func (s *CompatHelper) Open(name string) (c driver.Conn, err error) {
	sep := "?"
	if strings.Contains(name, "?") {
		sep = "&"
	}
	name += sep + "_mattn=true"
	return (*sqlite)(s).Open(name)
}

func init() {
	sql.Register("sqlite3", &CompatHelper{})
}
